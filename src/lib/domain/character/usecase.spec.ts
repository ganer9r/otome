...
describe('Character usecase TDD 테스트', () => {
	...
	describe('addCharacter(supabase, uid, dto)', () => {
		it('UUID v7 생성으로 캐릭터가 시간순 정렬 가능해야 한다', async () => {
			...
			// When - 순차적으로 캐릭터 생성
			const char1 = await addCharacter(supabase, testUid, dto1);
			testCharacterIds.push(char1.id);

			// 약간의 시간 지연 (UUID v7의 시간 구분을 위해)
			await new Promise((resolve) => setTimeout(resolve, 10));

			const char2 = await addCharacter(supabase, testUid, dto2);
		...
		it('캐릭터 생성이 성공해야 한다', async () => {
			...
			// When - 캐릭터 생성
			const character = await addCharacter(supabase, testUid, dto);
		...
		it('name이 필수값이므로 없으면 에러가 발생해야 한다', async () => {
			...
			// When & Then - 에러 발생
			await expect(addCharacter(supabase, testUid, dto)).rejects.toThrow();
		});
	});

	describe('getCharacters(supabase, uid)', () => {
		it('사용자별 캐릭터 목록을 조회해야 한다', async () => {
			...
			// When - 캐릭터 목록 조회
			const characters = await getCharacters(supabase, testUid);
		...
		it('생성일자 내림차순으로 정렬되어야 한다', async () => {
			// Given - 3개의 캐릭터 순차 생성
			const char1 = await addCharacter(supabase, testUid, { name: '첫번째' });
			testCharacterIds.push(char1.id);

			await new Promise((resolve) => setTimeout(resolve, 10));

			const char2 = await addCharacter(supabase, testUid, { name: '두번째' });
			testCharacterIds.push(char2.id);

			await new Promise((resolve) => setTimeout(resolve, 10));

			const char3 = await addCharacter(supabase, testUid, { name: '세번째' });
		...
		it('다른 사용자의 캐릭터는 조회되지 않아야 한다', async () => {
			...
			const otherChar = await addCharacter(supabase, otherUid, { name: '다른 사용자 캐릭터' });
	...
	describe('getCharacter(supabase, uid, id)', () => {
		it('ID로 단일 캐릭터를 조회해야 한다', async () => {
			...
			const created = await addCharacter(supabase, testUid, dto);
			testCharacterIds.push(created.id);

			// When - ID로 캐릭터 조회
			const character = await getCharacter(supabase, testUid, created.id);
		...
		it('존재하지 않는 ID는 null을 반환해야 한다', async () => {
			...
			// When - 존재하지 않는 캐릭터 조회
			const character = await getCharacter(supabase, testUid, nonExistentId);
		...
		it('타인의 캐릭터를 조회하려 하면 null을 반환해야 한다', async () => {
			// Given - 다른 사용자의 캐릭터
			const otherUid = randomUUID();
			const otherChar = await addCharacter(supabase, otherUid, { name: '타인 캐릭터' });
			testCharacterIds.push(otherChar.id);

			// When - 타인의 캐릭터 조회 시도
			const character = await getCharacter(supabase, testUid, otherChar.id);
	...
	describe('updateCharacter(supabase, uid, id, dto)', () => {
		it('부분 업데이트가 가능해야 한다', async () => {
			// Given - 캐릭터 생성
			const created = await addCharacter(supabase, testUid, {
				name: '원본 이름',
				info: '원본 정보',
				settings: '원본 설정'
			});
			...
			const updated = await updateCharacter(supabase, testUid, created.id, updateDto);
		...
		it('updated_at이 자동으로 업데이트되어야 한다', async () => {
			// Given - 캐릭터 생성
			const created = await addCharacter(supabase, testUid, { name: '캐릭터' });
			testCharacterIds.push(created.id);
			...
			// When - 캐릭터 업데이트
			const updated = await updateCharacter(supabase, testUid, created.id, {
				name: '변경된 이름'
			});
		...
		it('타인의 캐릭터는 수정할 수 없어야 한다', async () => {
			...
			// When & Then - 에러 발생
			await expect(
				updateCharacter(supabase, testUid, otherChar.id, { name: '변경 시도' })
			).rejects.toThrow();
		});
	});
	...
	describe('deleteCharacter(supabase, uid, id)', () => {
		it('캐릭터 삭제가 성공해야 한다', async () => {
			// Given - 캐릭터 생성
			const created = await addCharacter(supabase, testUid, { name: '삭제할 캐릭터' });
			testCharacterIds.push(created.id);

			// When - 캐릭터 삭제
			await deleteCharacter(supabase, testUid, created.id);
		...
		it('타인의 캐릭터는 삭제할 수 없어야 한다', async () => {
			...
			// When & Then - 에러 발생
			await expect(deleteCharacter(supabase, testUid, otherChar.id)).rejects.toThrow();
		});
		...
		it('존재하지 않는 캐릭터 삭제 시 에러가 발생해야 한다', async () => {
			...
			// When & Then - 에러 발생
			await expect(deleteCharacter(supabase, testUid, nonExistentId)).rejects.toThrow();
		});
	});
	...
	describe('통합 시나리오 테스트', () => {
		it('캐릭터 생성 → 조회 → 수정 → 삭제 전체 흐름이 정상 동작해야 한다', async () => {
			...
			// Step 2: 캐릭터 조회
			const fetched = await getCharacter(supabase, testUid, created.id);
			...
			// Step 4: 캐릭터 삭제
			await deleteCharacter(supabase, testUid, created.id);
			const deleted = await getCharacter(supabase, testUid, created.id);
		...
		it('여러 캐릭터를 생성하고 목록을 조회할 수 있어야 한다', async () => {
			...
			for (let i = 1; i <= 5; i++) {
				const char = await addCharacter(supabase, testUid, { name: `캐릭터${i}` });
			...
			// When - 캐릭터 목록 조회
			const list = await getCharacters(supabase, testUid);
