...
const SOURCE_DIR = path.join(PROJECT_ROOT, 'assets/ingredients2');
const OUTPUT_DIR = path.join(PROJECT_ROOT, 'static/imgs/ingredients2');
...
describe('모든 PNG -> WebP 변환', () => {
	it('일괄 변환', async () => {
		...
		console.log(
			`📊 용량: ${(totalSrc / 1024 / 1024).toFixed(1)}MB -> ${(totalOut / 1024 / 1024).toFixed(1)}MB (${((1 - totalOut / totalSrc) * 100).toFixed(0)}% 감소)`
		);
